...
@testset "StructuralSizer ↔ StructuralSynthesizer Workflow Integration" begin
    ...
    @testset "Flat plate sizing workflow (StructurePoint validation)" begin
        ...
        @test result.punching_check.passes  # Punching shear should pass
    ...
    @testset "Flat plate EFM sizing workflow" begin
        ...
        @test result.punching_check.passes
    ...
    @testset "Steel beam sizing workflow (skeleton → asap → size_members_discrete!)" begin
    ...
    @testset "Steel column sizing workflow (axial) (skeleton → asap → size_members_discrete!)" begin
        L_ft = 14.0
        L_m  = ustrip(u"m", L_ft * u"ft")
    ...
    @testset "Tributary loads workflow (building gen → initialize! → to_asap! → update_slab_loads!)" begin
        ...
        # Pressure should increase if we increase LL and call update_slab_loads!
        slab_idx = 1
