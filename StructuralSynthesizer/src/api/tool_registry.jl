...
const TOOL_REGISTRY = [
    Dict{String, Any}(
        ...
        "returns"           => "Dict with has_geometry, has_design, geometry (summary), params, health (all_pass, critical_ratio, critical_element, n_failing, failing_by_type {columns, beams, slabs, foundations}, embodied_carbon), session (n_designs, can_compare_deltas if ≥2 designs), has_trace. IMPORTANT: When n_failing > 0, always report failing_by_type counts to the user — critical_element is only the single worst failing element.",
    ...
    Dict{String, Any}(
        ...
        "description"       => "Returns the STRUCTURE-BASED GEOMETRY DIGEST: exact cell spans, slab panels, beam spans (with directional split), column heights, column tributary areas with grid regularity analysis, story heights, envelope dimensions, and structural flags. Computed by initializing a real BuildingStructure from the geometry (same pipeline as the solver). Use this tool when the system prompt geometry data seems stale, missing, or you need to refresh your understanding of the current geometry. Also works when the server has cached geometry from POST /design but no digest was in the prompt.",
        "phase"             => "orientation",
        "use_when"          => "You believe you lack geometry detail (spans, heights, tributary areas), or you want to double-check geometry data. Call this INSTEAD of saying geometry is unavailable.",
        "args"              => Dict{String, Any}(),
        "returns"           => "Dict with n_cells, n_slabs, n_beams, n_columns, stories (heights, source), cells (spans, areas, positions), slabs (spans, aspect ratios), beam_spans (all, x_direction, y_direction), column_heights, column_tributaries (min/max/mean, cv_overall, cv_within_position, by_position, grid_regularity, interpretation), envelope (footprint, height), structural_flags, plus a plaintext digest string.",
        "requires_design"   => false,
        "requires_geometry" => false,
    ),
    Dict{String, Any}(
    ...
    Dict{String, Any}(
        ...
        "args"              => Dict{String, Any}(),
        "returns"           => "Dict with by_type {column/beam/slab/foundation: {total, failing}}, top_critical (top 5 elements with governing_check, governing_ratio, ok), failure_breakdown (checks ranked by count), n_total_failing, size_warnings. Always cite by_type counts and top_critical to the user.",
        "requires_design"   => true,
        "requires_geometry" => false,
    ),
    Dict{String, Any}(
