...
const TOOL_REGISTRY = [
    Dict{String, Any}(
        ...
        "returns"           => "Dict with has_geometry, has_design, geometry_context, geometry OR geometry_availability, params, health (all_pass, critical_ratio, critical_element, n_failing, failing_by_type, embodied_carbon_kgco2e, materials {concrete_volume, steel_weight, rebar_weight}, embodied_carbon_by_system_kgco2e?), session (n_designs, can_compare_deltas, latest_passed), has_trace, _guidance. When n_failing > 0 always report failing_by_type counts.",
    ...
    Dict{String, Any}(
        ...
        "args"              => Dict{String, Any}(),
        "returns"           => "Dict with by_type {column/beam/slab/foundation: {total, failing}}, top_critical (top 5), failure_breakdown, n_total_elements, n_total_failing, note, size_warnings?, _guidance?, plus from design summary: all_pass, critical_element, critical_ratio, total_ec_kgco2e, floor_area, ec_intensity, embodied_carbon_by_system_kgco2e (slabs/columns/beams/struts/foundations/fireproofing), embodied_carbon_note (total vs per-element get_diagnose). Always cite by_type counts and top_critical.",
        "requires_design"   => true,
        "requires_geometry" => false,
    ),
    Dict{String, Any}(
    ...
    Dict{String, Any}(
        ...
        "returns"           => "Dict with ok, quick_check, all_pass, critical_ratio, critical_element, embodied_carbon_kgco2e, n_failing, summary (text), warnings, applied_params, note?, _guidance?.",
        "requires_design"   => false,
        "requires_geometry" => true,
    ),
    Dict{String, Any}(
        "name"              => "compare_designs",
        "description"       => "Delta table: **previous** vs **latest** entry in session design history (no indices). Includes geometry_hash per side, cross_geometry_comparison, mechanism_shift, changed_params.",
        "phase"             => "exploration",
        "use_when"          => "After run_design or when the user asks what changed vs the last run. Always pairs the immediately previous completed design with the latest. For older runs, use get_design_history to list entries only — this tool does not pick arbitrary indices. Cite comparison_note when cross_geometry_comparison is true.",
        "args"              => Dict{String, Any}(),
        "returns"           => "Dict with comparison_mode=previous_vs_current, previous/current (history_index, all_pass, critical_element, embodied_carbon, n_failing, geometry_hash, source), deltas (pass_improved, pass_regressed; values are current−previous), changed_params, mechanism_shift?, cross_geometry_comparison, comparison_note?, _guidance?.",
        "requires_design"   => true,
        "requires_geometry" => false,
    ),
    Dict{String, Any}(
    ...
    Dict{String, Any}(
        ...
        "description"       => "Plain-English comparison of **previous vs latest** session run (same pairing as compare_designs).",
        "phase"             => "communication",
        "use_when"          => "After compare_designs or when the user wants a narrative of what changed vs the last run. Requires ≥2 history entries.",
        "args"              => Dict{String, Any}(
            "audience" => Dict(
                "type" => "string",
                "required" => true,
...
const _LLM_CONTRACT_VERSION = "1.2.2"
...
function api_llm_contract()::Dict{String, Any}
    ...
    return Dict{String, Any}(
        ...
        "workflow_sequence" => [
            ...
            "compare_designs (previous vs latest, no args); record_insight",
        ],
    )
end
